...
export default function QuizPage() {
  ...
  const progressText = useMemo(
    () => `${Math.min(currentIndex + 1, quizQuestions.length)} / ${quizQuestions.length}`,
    [currentIndex]
  );
    ...
    return (
      <main className="mx-auto flex min-h-[calc(100vh-80px)] max-w-3xl items-center px-4 py-12 sm:px-6">
        <section className="w-full rounded-lg border border-forest-100 bg-white p-8 text-center shadow-soft">
          ...
          <h1 className="mt-3 text-3xl font-bold text-forest-900">总分：{score} / {quizQuestions.length}</h1>
          <p className="mx-auto mt-4 max-w-xl leading-7 text-forest-800">
            已完成本轮基础题。可以回到首页复习知识卡片，也可以重新开始巩固易错点。
          </p>
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:py-14">
      <div className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
        <div>
          <p className="text-sm font-semibold text-forest-700">简单自测题</p>
          <h1 className="mt-2 text-3xl font-bold text-forest-900">检验基础概念掌握情况</h1>
        </div>
      ...
      <div className="mt-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        ...
        >
          {currentIndex === quizQuestions.length - 1 ? "查看总分" : "下一题"}
        </button>
      </div>
    </main>
  );
